...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        auth = CognitoAuth(apikey=data["api_key"])
        device = await hass.async_add_executor_job(
            CHSesame2, auth, data["uuid"], data["secret_key"]
        )
    ...
    # InvalidAuth
    _LOGGER.info(f"[Initial MechStatus] {device.getDeviceShadowStatus()}")
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        try:
            ...
            await self.async_set_unique_id(user_input["uuid"].replace("-", ""))
